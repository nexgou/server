...
The main framework package. Provides the application bootstrap, HTTP engine, module system, IoC container, Dependency Injection, Controllers, Guards, Interceptors, Pipes, Exception Filters, `ConfigModule`, `LogModule`, and the `nexgoutest` testing utilities.
...
- Choose the most efficient wire format per use case (JSON for APIs, MessagePack for queues, Protocol Buffers for internal services).
